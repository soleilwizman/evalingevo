...
['Raw Evo association','One raw Evo interaction score is compared with measured epsilon','Spearman 0.0176\n95% interval −0.0210 to 0.0533','Do the two rankings move together? Almost not at all.'],
['Raw Evo association','The numerical Evo score is compared with measured epsilon','Pearson 0.0011\n95% interval −0.0384 to 0.0414','Is there a linear relationship? Almost not at all.'],
['Calibrated Evo','A fold-specific line uses Evo score to predict epsilon','RMSE 0.35427','How close are Evo-based numerical predictions?'],
['Zero-interaction reference','Predict epsilon = 0 for every pair','RMSE 0.35376','How well does the simplest no-interaction guess do?'],
['Training-fold mean reference','Predict the training-fold average epsilon for every held-out pair','RMSE 0.35391','How well does a constant average do?'],
['Sequence-only ridge','Use DNA word counts and positions to predict epsilon directly','RMSE 0.37337','Does a simple sequence model help? Here, no.']],[1.55,2.15,1.55,1.55])
...
p('Across all 2,833 held-out predictions, calibrated Evo has RMSE 0.35427, while the zero-interaction reference has RMSE 0.35376. The difference is 0.00051 log2-activity units, with Evo slightly worse. The same ordering occurs in all five held-out folds, and the paired cluster-bootstrap interval for zero-reference RMSE minus Evo RMSE is −0.00083 to −0.00024. Thus this run provides no positive RMSE gain for Evo; the practical difference is very small.')
p('The central finding is that the four-way Evo likelihood difference does not usefully order the measured interactions in this dataset. A Spearman value of 0.0176 is the correlation between those two rankings; it is not a percentage of correctly predicted interactions. The interval includes zero, and the calibrated model also fails to improve prediction error over zero or the training mean. [5]')
p('Beating the sequence-only ridge does not rescue the result: the ridge itself is worse than the simpler references. The appropriate conclusion is that the tested Evo readout has not demonstrated useful prediction of this assay’s interaction measurements.')
...
p('Dividing the upper endpoint of the raw Spearman interval, 0.05325, by 0.53468 gives 0.09959. We recorded this as an approximate noise-adjustment diagnostic. However, the classical attenuation formula applies to Pearson correlation, and our calculation holds the estimated reliability fixed. It neither supplies a general correction for ranks nor propagates uncertainty in reliability. Consequently, 0.10 is a sensitivity estimate, not a rigorous upper confidence bound on the true Spearman correlation. [3, 4]')
...
out=Path('/Users/soleliwizman/evalingevo/docs/Evo2_regulatory_interactions_proposal_and_MVP.docx')
D.save(out)
print(out)
