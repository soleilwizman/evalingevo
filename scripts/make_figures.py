...
def probe_value(path, label):
    ...
    if not match:
        raise SystemExit(f"{path}: no row '{label}'. Re-run the probe; the file may be truncated.")
    return float(match.group(1))


def probe_margin(path):
    match = re.search(
        r"probe minus (?:word counts|k-mers): ([+-][\d.]+)\s+"
        r"95% interval \[([+-][\d.]+), ([+-][\d.]+)\]",
        Path(path).read_text(),
    )
    if not match:
...
LAYER_NOTE = (
    "Probes read one frozen layer, mean pooled unless stated: Evo 2 blocks.26.mlp.l3 (width 4096); "
...
def collect(results):
    ...
    element = [
        (
            ...
            probe_value(evo, "Evo score (1 feature)"),
            "likelihood",
        ),
        ("GC content", probe_value(evo, "GC content (1 feature)"), "baseline"),
        (
            "Evo 2 probe, last token",
            probe_value(evo_last, "Evo hidden layer blocks.26.mlp.l3 (probe)"),
            "probe",
        ),
        ...
        (
            "NTv3 650M probe + k-mers",
            probe_value(n650, "NTv3 hidden layer + word counts"),
            "combined",
        ),
        (
            "NTv3 100M probe + k-mers",
            probe_value(n100, "NTv3 hidden layer + word counts"),
            "combined",
        ),
        (
            "Evo 2 probe, mean pooled",
            probe_value(evo, "Evo hidden layer blocks.26.mlp.l3 (probe)"),
            "probe",
        ),
        (
            "Evo 2 probe + k-mers",
            probe_value(evo, "Evo hidden layer blocks.26.mlp.l3 (probe) + word counts"),
            "combined",
        ),
    ]
    element_margins = [
    ...
    single = [
        ...
        (
            ...
            e["model_delta_score"]["spearman"],
            "likelihood",
        ),
        (
            "NTv3 delta pseudo-log-likelihood",
            n["model_delta_score"]["spearman"],
            "likelihood",
        ),
        ...
        (
            "delta k-mers, words made and broken",
            e["kmer_delta"]["spearman"],
            "baseline",
        ),
    ]
    single_margins = []
    ...
    ceiling, n_elements = element_ceiling(
        results / "evo2_7b_base/predictions.csv", Path("data/audit.csv.gz")
    )
    ...
    return {
        "element": {
            "rows": element,
            "margins": element_margins,
            "baseline": kmer,
            "baseline_name": "1/2/3-mer counts",
            "ceiling": ceiling,
            "n": n_elements,
        ...
        "single": {
            "rows": single,
            "margins": single_margins,
            "baseline": e["kmer_delta"]["spearman"],
            "baseline_name": "delta k-mers",
            "ceiling": evo_sv["zero_shot"]["ceiling"],
            "n": evo_sv["zero_shot"]["n"],
...
def bar_panel(axis, panel, note=None):
    ...
    for index, value in enumerate(values):
        axis.text(
            value + (pad * 0.4 if value >= 0 else -pad * 0.4),
            index,
            f"{value:+.3f}",
            va="center",
    ...
    axis.text(
        panel["baseline"],
        len(rows) - 0.15,
        f"baseline to beat: {panel['baseline_name']}  "
        if near_right
    ...
    if pair:
        ...
        if len(seats) == 2:
            ...
            axis.text(
                x + tick,
                (bottom + top) / 2,
                f"  not distinguishable\n  {pair['difference']:+.3f} [{low:+.3f}, {high:+.3f}]",
                va="center",
                ha="left",
                fontsize=8,
                color=MUTED,
...
def legend(figure, kinds):
    handles = [
        plt.Line2D([], [], marker="s", ls="", ms=9, color=COLOR[k], label=KIND_LABEL[k])
        for k in kinds
    ]
    figure.legend(
        handles=handles,
...
def main():
    ...
    for axis in axes:
        axis.set_xlabel(
            "out-of-fold Spearman correlation with the measurement",
            fontsize=9,
            color=MUTED,
        )
    figure.suptitle(
        "Frozen genomic language models against cheap sequence baselines",
        fontsize=13,
        color=INK,
        x=0.012,
        ha="left",
    ...
    figure, axes = plt.subplots(3, 1, figsize=(11.4, 7.6), gridspec_kw={"height_ratios": [4, 2, 2]})
    figure.patch.set_facecolor("white")
    margin_panel(
        axes[0],
        data["element"]["margins"],
    ...
    for axis in axes:
        axis.set_xlabel(
            "difference in out-of-fold Spearman, 95% interval over region groups",
            fontsize=9,
            color=MUTED,
        )
    figure.suptitle(
        "Does the model beat the baseline? Anything crossing zero does not.",
        fontsize=13,
        color=INK,
        x=0.012,
        ha="left",
    ...
    axis.set_xlabel(
        "out-of-fold Spearman correlation with the measurement", fontsize=9, color=MUTED
    )
    handles = [
        plt.Line2D([], [], marker="s", ls="", ms=9, color=COLOR[k], label=KIND_LABEL[k])
