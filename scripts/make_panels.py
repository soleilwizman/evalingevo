...
Out-of-fold predictions are saved under <out>/cache for inspection. Unversioned
cached fits are not reused, because they may have different inputs or CV rules.
...
def cached(cache, name, build):
    """Write intermediate predictions; unversioned files are never reused for fits."""
...
def single_variant_panels(cache):
    ...
    delta_kmers = cached(cache, "sv_delta_kmers", lambda: kmers(seqs) - kmers(refs))

    def fit(x):
        # Match the single-variant readout: ridge only for multi-column features.
        return out_of_fold_linear(x, y, groups, 5, ridge=x.shape[1] > 1, seed=0)

    fits = {
        "delta k-mers": (
            cached(cache, "sv_pred_delta_kmers", lambda: fit(delta_kmers)),
            "baseline",
        ),
        "allele identity": (
            cached(cache, "sv_pred_allele", lambda: fit(allele_features(evo))),
            "baseline",
        ),
        "GC content": (
            cached(cache, "sv_pred_gc", lambda: fit(gc_fraction(seqs).reshape(-1, 1))),
            "baseline",
        ),
        "Evo 2 likelihood": (
            cached(
                cache,
                "sv_pred_evo",
                lambda: fit(evo.delta_score.to_numpy().reshape(-1, 1)),
            ),
            "likelihood",
        ),
        "NTv3 likelihood": (
            cached(
                cache,
                "sv_pred_ntv3",
                lambda: fit(ntv3.delta_score.to_numpy().reshape(-1, 1)),
            ),
            "likelihood",
        ),
    }
    ceiling = json.loads(Path("results/evo2_7b_base/single_variant.json").read_text())["zero_shot"][
        "ceiling"
    ]
    return {
        "y": y,
        ...
        "scatters": [
            (
                "Evo 2 delta log-likelihood",
                evo.delta_score.to_numpy(),
                "raw score change",
            ),
            (
                "NTv3 delta pseudo-log-likelihood",
                ntv3.delta_score.to_numpy(),
                "raw score change",
            ),
            (
                "delta k-mer counts, fitted",
                fits["delta k-mers"][0],
...
def element_panels(cache):
    ...
    def probe_prediction(directory, pooling="mean"):
        ...
        validate_elements(table, el.set_index("sequence_id"))
        matrix = load_matrix(Path(directory) / f"X_{pooling}.npy", table, ["sequence_id"])
        position = {sequence: row for row, sequence in enumerate(table.sequence_id)}
        missing = [s for s in order if s not in position]
        if missing:
            raise SystemExit(
                f"{directory}: {len(missing)} elements have no embedding; re-run embed"
            )
    ...
    fits = {
        "GC content": (
            cached(
                cache,
                "el_pred_gc",
                lambda: out_of_fold(gc_fraction(seqs).reshape(-1, 1), y, groups, 5),
            ),
            "baseline",
        ),
        "1/2/3-mer counts": (
            cached(cache, "el_pred_kmers", lambda: out_of_fold(kmers(seqs), y, groups, 5)),
            "baseline",
        ),
        "Evo 2 likelihood": (
            cached(
                cache,
                "el_pred_evo_score",
                lambda: out_of_fold(el.s_wt.values.reshape(-1, 1), y, groups, 5),
            ),
            "likelihood",
        ),
        "Evo 2 probe": (
            cached(
                cache,
                "el_pred_evo_probe",
                lambda: probe_prediction("results/evo_probe"),
            ),
            "probe",
        ),
        "NTv3 100M probe": (
            cached(
                cache,
                "el_pred_ntv3_100m",
                lambda: probe_prediction("results/ntv3_100m_final"),
            ),
            "probe",
        ),
        "NTv3 650M probe": (
            cached(
                cache,
                "el_pred_ntv3_650m",
                lambda: probe_prediction("results/ntv3_650m_final"),
            ),
            "probe",
        ),
    }
    return {
        ...
        "scatters": [
            (
                "Evo 2 probe, mean pooled",
                fits["Evo 2 probe"][0],
                "out-of-fold prediction",
            ),
            (
                "1/2/3-mer counts, fitted",
                fits["1/2/3-mer counts"][0],
                "out-of-fold prediction",
            ),
...
def draw(panel, path):
    ...
    bars = axes[-1]
    scored = [
        (name, *interval(pred, y, groups), kind) for name, (pred, kind) in panel["bars"].items()
    ]
    ...
    for index, (_, value, _, high, _) in enumerate(scored):
        bars.text(index, high + 0.018, f"{value:+.3f}", ha="center", fontsize=8, color=INK)
    if panel["ceiling"]:
        bars.axhline(panel["ceiling"], color=MUTED, lw=1, ls=(0, (5, 4)), zorder=2)
        bars.text(
            len(scored) - 0.45,
            panel["ceiling"],
            f"ceiling {panel['ceiling']:.2f} ",
            fontsize=8,
            color=MUTED,
            va="bottom",
            ha="right",
        )
    bars.set_xticks(x, ["\n".join(textwrap.wrap(r[0], 11)) for r in scored], fontsize=8, color=INK)
    bars.set_ylabel("out-of-fold Spearman", fontsize=9, color=INK)
    bars.set_title(
        "Every readout, one protocol\ngrouped five-fold, 95% interval",
        fontsize=9.5,
        color=INK,
    )
