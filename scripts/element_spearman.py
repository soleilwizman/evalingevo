...
def build(evo_predictions, ntv3_predictions, probe_dir, folds=5, n_boot=1000, seed=0):
    ...
    validate_elements(table, evo.set_index("sequence_id"))
    matrix = load_matrix(Path(probe_dir) / "X_mean.npy", table, ["sequence_id"])
    ...
    for label, features, kind in readouts:
        prediction = out_of_fold(features, y, groups, folds, seed)
...
def draw(data, path):
    ...
    axis.set_ylabel("Spearman correlation with measured element activity", fontsize=10, color=INK)
    axis.set_ylim(
        min(0, min(r["ci"][0] for r in rows)) - 0.05,
    ...
    axis.set_title(
        f"Predicting element activity from the same 200 bases  "
        f"(n = {data['n']:,} elements)\n"
