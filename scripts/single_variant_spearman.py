...
def find_variant_probe(path, label):
    ...
    if j.get("protocol") != PROTOCOL:
        raise ValueError(f"{p}: refit the variant probe under {PROTOCOL}")
    for r in j["readouts"]:
...
def build(folds=5, seed=0, n_boot=1000):
    ...
    found = {
        "NTv3 650M score": (
            find_score,
            "results/ntv3_650m_pre/single_variant.json",
            MISSING_SCORE,
        ),
        "NTv3 100M probe": (
            find_variant_probe,
            "results/ntv3_100m_variants/variant_probe.json",
            MISSING_PROBE,
        ),
        "NTv3 650M probe": (
            find_variant_probe,
            "results/ntv3_650m_variants/variant_probe.json",
            MISSING_PROBE,
        ),
        "Evo 2 probe": (
            find_evo_variant_probe,
            "results/vp_evo2/probe.txt",
            MISSING_PROBE,
        ),
    }
    order = [
    ...
    for label in order:
        ...
        if row is None:
            ...
            interval = (
                ...
                if row["ci"]
                else "  (no interval, other protocol)"
            )
...
def draw(data, path):
    ...
    axis.set_title(
        f"Predicting the effect of a single substitution  "
        f"(n = {data['n']:,} variants)\n"
    ...
    if noci:
        note = (
            "* Evo 2's variant probe predates this protocol and its embeddings were "
...
def main():
    ...
    parser.add_argument("--out", default="results/figures/single_variant_spearman.png", type=Path)
