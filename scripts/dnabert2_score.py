...
from model_runtime import pick_device

SCORE_DEFINITION = (
    "mean(forward,RC) of summed masked-LM pseudo-log-likelihood over the BPE tokens "
...
class DNABERT2Scorer:
    ...
    def __init__(self, checkpoint="zhihan1996/DNABERT-2-117M", revision=None, device="auto"):
        ...
        self.tokenizer, self.model, self.device = load_hf(
            checkpoint, revision, device, family="dnabert2"
        )
    ...
    def _tokens(self, sequence):
        ...
        if recovered != sequence:
            raise ValueError(
                "DNABERT-2 tokens do not spell the input back "
    ...
    def _pseudo_log_likelihood(self, sequence):
        ...
        for start in range(0, len(real), self.TOKEN_CHUNK):
            ...
            if logits.shape[:2] != batch.shape:
                raise ValueError(
                    f"Unexpected DNABERT-2 logits shape {tuple(logits.shape)} "
...
def main():
    ...
    result = score_quartets_dnabert2(
        args.quartets,
        args.output,
        args.checkpoint,
        args.revision,
