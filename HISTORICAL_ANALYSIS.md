...
> **Protocol migration:** The numerical results below and committed `results/` artifacts
> are historical. The code now uses nested genomic-group CV, fold-local scaling and
> real-base-only pooling. No historical result has been overwritten or represented as
> regenerated. See [PROTOCOL.md](PROTOCOL.md) for corrections, validation commands and
> remaining GPU checks, and the [embedding runbook](RUNBOOK_variant_embeddings.md).
...
Frozen embeddings do carry element activity that letter counting misses, and which layer you read matters more than which model you read. Probing out of fold on the same grouped folds against the same 0.456 word-count baseline, Evo 2's blocks.26.mlp.l3 reaches 0.505, a margin of +0.049 (interval +0.015 to +0.081). NTv3 650M read at its transformer bottleneck reaches 0.440, a margin of -0.016 (-0.048 to +0.015), because seven downsamples leave a 200-base oligo as two positions there. Read at the top of its deconvolution tower, one vector per base, it reaches 0.485, but that +0.029 margin sits on the boundary: its lower bound changes sign with the bootstrap seed, clearing zero in 7 of 10. Read in the early convolution tower instead, at conv_2 and 128 positions, it reaches 0.566, a margin of +0.109 (+0.081 to +0.138), the largest of any representation we tested and more than double Evo 2's. Two caveats: that layer was chosen after seeing the per-layer curve, which inflates the winner, and 18 of the 26 layers in our 650M sweep are non-finite and need re-running, including every transformer block, so the middle of the U is unmeasured. All of this predicts reference activity rather than variant interaction, which is the harder question the probe in Section 5 is meant to reach.
