...
We also probed the frozen embeddings out of fold on the same grouped folds, and the layer you read matters more than the model you read. Against the 84-number baseline above (rank correlation 0.456), Evo 2's blocks.26.mlp.l3 reaches 0.505, a margin of +0.049 (interval +0.015 to +0.081). NTv3 650M read at its transformer bottleneck reaches 0.440, a margin of -0.016, because seven downsamples leave a 200-base oligo as two positions there. Read at the top of its deconvolution tower, one vector per base, it reaches 0.485. Read in the early convolution tower instead, at conv_2 and 128 positions, it reaches 0.566, a margin of +0.109 (+0.081 to +0.138).

Those margins are measured against a weak baseline, and raising it reverses the conclusion. Counting 4-mers as well lifts the baseline to 0.503, 5-mers to 0.529, and the full set of 1 through 6-mers to 0.538, which is above every probe we ran including Evo 2's 0.505. Against that strongest baseline nothing wins: Evo 2 falls to -0.033, NTv3's deconvolution output to -0.054, its transformer bottleneck to -0.098, and conv_2 to +0.027 with only 1 of 10 bootstrap seeds clearing zero. Evo 2's +0.049 over the 84-number baseline was an artifact of that baseline; it is already down to +0.002 against 4-mers. NTv3's conv_2 stays ahead on four of the five rungs and is level on the richest, and its layer was chosen after seeing the per-layer curve, which inflates the winner. The honest summary is that these models' element-level representations are level with or worse than counting short words, with NTv3's early convolution tower the closest thing to an exception. Two further limits: 18 of the 26 layers in our 650M sweep are non-finite and need re-running, including every transformer block, so the middle of the U-Net is unmeasured; and all of this predicts reference activity rather than variant interaction, which is the harder question the probe in Section 5 is meant to reach.
