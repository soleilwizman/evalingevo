...
b) Variant scoring: what does changing one base do?
...
Probing out of fold on the same grouped folds against the same 0.462 word-count baseline, Evo 2's blocks.26.mlp.l3 reaches 0.505, a margin of +0.043 (interval +0.015 to +0.081) better than k-mers alone. NTv3 650M read at its transformer bottleneck reaches 0.440, a margin of -0.016 (-0.048 to +0.015), as seven downsamples leave a 200-base oligo as two positions there. Upon realizing how heavily NTv3 would downsample on a 200 bp sequence, we pivoted. Read at the top of its _deconvolution tower_, one vector per base, it reaches 0.485. Read in the early convolution tower instead, at conv_2 and 128 positions, it reaches 0.566, a margin of +0.109 (+0.081 to +0.138), the largest of any representation we tested and more than double Evo 2's. All of this predicts reference activity rather than variant interaction. A more thorough sweep of Evo2's layers will be required to determine whether there is a better layer to run the same experiment on -- particularly layer 28.

Comparing predicted score to measured experimental sequence activity, Evo 2's log-likelihood correlates with measured reference activity at Spearman -0.018, cluster-bootstrap interval -0.060 to +0.025, while NTv3 gives -0.088, interval -0.130 to -0.043, which is weakly negative rather than flat. On single variants, Spearman -0.005 (CI -0.034 to +0.022) against Evo's -0.006. On variant interaction, Evo 2 gives Spearman +0.021 (interval -0.019 to +0.059) and NTv3 +0.021 (-0.017 to +0.059). NTv3's U-Net requires a sequence length divisible by 128, so each oligo was padded symmetrically to 256 with N, and only the 200 real positions were scored. 
