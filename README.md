Draft Proposal (WIP): Can Evo 2 predict the effect of single regulatory variants (and what explains its performance)?
...
(B) Aims
Aim 1: Test whether Evo’s existing sequence scores predict measured single-variant effects in regulatory sequence.

The primary benchmark is the effect of one substitution on the activity of the element that contains it. We started on the two-variant interaction contrast and moved off it for a measurement reason rather than a preference. The second difference is mostly assay noise: its reliability is 0.286, which caps any predictor at an observed correlation of 0.535, and only 58 of 2,833 pairs carry the interaction flag. The single-variant effect, taken from the same quartets, has reliability 0.731, a ceiling of 0.855, and 522 flagged variants across 5,666 observations. A null result on the interaction contrast is close to uninformative; a null on single variants is evidence. The interaction analysis is kept as a secondary result and is reported with its ceiling attached.

One simplification follows. Siraj et al. recode alleles by measured activity, and that recoding exists only to put the four-haplotype contrast on their convention. It has no bearing on a single substitution, so the primary benchmark runs on plain ref/alt coding and the recoding drops out of the main path.
...
Evo does not predict single-variant effects, and the benchmark is now well powered enough for that to mean something. Each quartet contains two single-substitution sequences, giving 5,666 single variants in 2,251 region groups, each with a measured log2 activity change relative to its own reference 200-mer and a standard error. The variance of those effects is 0.166 against a mean squared standard error of 0.045, so reliability is 0.731 and a perfect predictor would reach an observed correlation of 0.855. Every number below is reproduced by `python3 single_variant.py --predictions results/evo2_7b_base/predictions.csv --out results/evo2_7b_base/single_variant.json`.

Evo's change in log-likelihood, s(alt) - s(ref), gives signed Spearman -0.018 against the measured change, cluster-bootstrap interval -0.048 to +0.010. Comparing sizes rather than signs gives -0.006, interval -0.034 to +0.022. Both intervals contain zero, against a ceiling of 0.855.

The baseline that does carry signal is the set of DNA words the substitution creates and destroys. Counting 1, 2 and 3-mers in the alternate sequence and subtracting the same counts in the reference gives a sparse 84-number vector; ridge on it, out of fold under the grouped five-fold split, reaches +0.177. Adding Evo's score to that vector gives +0.176, no gain. The identity of the reference and alternate base alone reaches +0.082. Predicting the training-fold mean gives -0.034. On error, the training mean gives RMSE 0.4075 and the best model 0.4014, against a floor of 0.2111 for a predictor limited only by measurement error, so most of the resolvable range is still unclaimed by anything here.

Of the 5,666 variants, 522 (9.2%) carry the source study's emVar flag. Ranking by the size of Evo's score change gives AUROC 0.504, which is chance. GC content of the alternate sequence gives 0.584. An out-of-fold logistic regression on GC and position reaches 0.576, and adding Evo's score moves it to 0.574, a change of -0.002 with an interval of -0.005 to +0.001. Across the 2,595 distinct reference 200-mers, Evo's whole-sequence score against the measured reference activity gives Spearman -0.018 (CI -0.060 to +0.025), whereas counting G and C in the same 200 bases gives +0.328. In another baseline looking at 1, 2, 3 k-mers, for each 200-base sequence we counted how often each single-letter string appears (4 numbers), each two-letter string (16), and each three-letter string (64) giving 84 numbers per sequence. Fitting ridge regression on those 84 numbers, out of fold under the same grouped five-fold split, it correlated with enhancer activity at rank correlation 0.458.


Notably, as Evo 2 is autoregressive, we repeated everything with Nucleotide Transformer v3, a masked language model, using the frozen NTv3_100M_pre checkpoint. To score, we masked one base at a time and added the log probability of the base that is actually there, across all 200 positions, averaged over forward and reverse-complement, just as was done on Evo. Comparing predicted score to measured experimental sequence activity, Evo 2's log-likelihood correlates with measured reference activity at Spearman -0.018, cluster-bootstrap interval -0.060 to +0.025, while NTv3 gives -0.088, interval -0.130 to -0.043, which is weakly negative rather than flat. On single-variant effects NTv3's signed Spearman is -0.038, interval -0.069 to -0.012, which excludes zero on the wrong side: weakly anti-correlated rather than flat. Its magnitude correlation is -0.005, interval -0.034 to +0.022. Adding it to the delta k-mer baseline gives +0.177 against +0.177, and its emVar AUROC is 0.483. On the secondary interaction contrast, Evo 2 gives Spearman +0.021 (interval -0.019 to +0.059) and NTv3 +0.021 (-0.017 to +0.059), both against the much lower ceiling of 0.535. NTv3's U-Net requires a sequence length divisible by 128, so each oligo was padded symmetrically to 256 with N, and only the 200 real positions were scored. 
...
The primary eval asks whether Evo can predict the effect of a single substitution across the 5,666 single variants in this quality-controlled subset of Siraj’s data. A tighter version asks the same question on the 522 variants the source study flagged as activity-changing, where the effects are large enough to rank. The interaction analysis remains as a secondary result, and any claim from it has to be read against its 0.535 ceiling. 
